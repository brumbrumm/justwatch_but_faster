...
Future<Movie> fetchMovie(String type, int id) async{
  print(id);
  print(type);
  final String url = "https://apis.justwatch.com/content/titles/$type/$id/locale/de_DE?language=de";
...
Future<List<Provider>> fetchProvider() async{
  final String url = "https://apis.justwatch.com/content/providers/locale/de_DE";
  final response = await retry(
        () => http.get(
        url, headers: {
      //'User-Agent':	'SimpleWin',
...
Future<List<Movie>> fetchPopular(List<String> providers, int pageNumber) async{
  ...
  final String prefixUrl = 'https://apis.justwatch.com/content/titles/de_DE/popular';
